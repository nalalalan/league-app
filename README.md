...
The live recorder is intended to run without Codex in the loop. It watches for the League game process and the local League Client gameflow state, captures the screen at low priority, writes a sidecar next to each auto clip, rejects incomplete, black-screen, or broken captures before publish, then lets the publisher analyze, sync, verify, and deploy the recording. The default installed mode is full-desktop capture so alt-tab and focus changes do not stop the game record; the sidecar states when desktop capture was used. Offline checks handle capture validity and media prep; low-motion captures are allowed to be small as long as the segments are real video. OpenAI is used only for the coaching narrative after the clip passes those checks.

The site exposes a compact recorder heartbeat at `/api/recording-status`. The local recorder and publisher post `watching`, `recording`, `processing`, `publishing`, `published`, `blocked`, or `error`, plus a coarse progress value, so the recordings header can show whether a game is being captured, analyzed, deployed, or live before the final review appears. Automatic captures with a recorder sidecar are accepted when the sidecar proves enough real segment coverage, even if the compressed desktop output is small.
