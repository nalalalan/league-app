...
The live recorder is intended to run without Codex in the loop. It watches for the League game process and the local League Client gameflow state, captures the League window area at low priority, writes a sidecar next to each auto clip, rejects incomplete, black-screen, or DPI-cropped captures before publish, then lets the publisher sync and deploy the recording. Region capture pauses instead of recording browser/desktop content while League is not foreground, and any alt-tab-touched boundary segments are excluded from the published review clip. Offline checks handle capture validity and media prep; low-motion window captures are allowed to be small as long as the segments are real video. OpenAI is used only for the coaching narrative after the clip passes those checks.
