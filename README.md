...
The live recorder is intended to run without Codex in the loop. It watches for the League game process and the local League Client gameflow state, captures the League window region at low priority, writes a sidecar next to each auto clip, rejects incomplete, black-screen, or broken captures before publish, then lets the queued publisher analyze, sync, verify, and deploy one post-game review at a time while the next game can continue recording. If the recorder restarts mid-game, it ignores that partial game and waits for the next full one. Offline checks handle capture validity and media prep; low-motion captures are allowed to be small as long as the segments are real video. OpenAI is used only for the coaching narrative after the clip passes those checks.
