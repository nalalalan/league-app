...
The site organizes knowledge by situations rather than by abstract lessons. The current version makes those situations champion-specific and removes the intermediate public labels that had made the cards feel misleading. Selecting Samira now shows one-sentence prompts for stressful moments: wanting to press E because a fight feels urgent, not recognizing the crowd control that can stop the dash, chasing a low-health enemy into fog, pressing W because proximity feels dangerous, staying after R because the damage felt good, or trying to W a tower shot. Each prompt is followed by one combined response paragraph that explains the action, the reason it matters, and the repetition to practice. Selecting Caitlyn shifts the same practice-room idea toward next safe minion, leaving when noticed, recovering after poke, and shooting the closest safe target. This format was chosen because the user needed enough detail to recognize the exact tricky moment without reading a giant rule wall or a card taxonomy.
...
The seed corpus was the user-provided League learning arc pasted into the implementation thread on May 17, 2026. It described the user's own League practice, including early Caitlyn farming goals, Samira drills, bot-ladder practice, camera and sensory adjustments, death exposure, and champion-fit comparisons. A later May 17, 2026 prompt added a Samira crowd-control guide as a source for the page's CC-specific situation. The implementation treated these texts as the current source of truth for the public situation notes. Match statistics mentioned in the source were treated as user-supplied examples, not independently verified performance results.
...
The website is served by a small Node application on Railway. It serves the hand-authored page and paper artifacts from the public directory and exposes \texttt{/api/logs} for public note reads and writes. On Railway, persistent storage is provided through the configured volume path, and writes are gated by \texttt{LEAGUE\_WRITE\_TOKEN}. This keeps public notes browser-independent without adding a fake local-only note form or claiming Riot API integration that is not present in version 1.
...
The first public data source is the League practice arc supplied in the implementation thread and the public log exposed through \href{https://league.aolabs.io}{league.aolabs.io}. Raw game screenshots were not included in this first version.
