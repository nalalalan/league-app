...
\noindent The May 24 key-click update makes the action-script gate stricter: every new current-standard full-game row must start its main review with one key timestamp sentence that names the exact visible state, the mistake category, and the correct next click. Rows are rejected if they fall back to generic phrases such as a current-match review frame or only tell the player to run a broad branch.
