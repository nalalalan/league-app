...
The public page is not a neutral wrapper around the recordings. It is part of the intervention being studied. For that reason, the paper records the actual interface, not only the backend, manifest, or coaching text. The current live page begins with the shared AO Labs header, a quiet paper strip, and one Samira note-intake card. The earlier champion-card surface used exact user-supplied photographs for Samira, Caitlyn, and Fizz, and those source assets remain in the project archive, but they are no longer part of the default home-page task. This matters because the page is meant to feel like a low-friction intake surface rather than a generic analytics product.

The older top review tile combined a selected champion's rank trend plot and latest synced state into one calm surface. That tile now acts as source history rather than the default interface. The current default card accepts long pasted Samira note blocks, stores them in the same persisted note corpus, and leaves the compact approximate rank read plus current Samira tips to \texttt{/api/samira} and the morning queue. The home page therefore answers one question first: where does the next Samira note go?
...
\texttt{/api/samira} read & Feeds rank estimate and morning-email tips from notes plus source history & Describe the source-bounded rank/tip contract without showing it as a home-page dashboard \\
