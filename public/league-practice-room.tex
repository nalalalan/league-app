...
The recording review does not attempt to become a full coaching dashboard. The public surface now keeps champion cards first, then gives one Samira focus, then opens a large selected recording player with a compact recording table underneath. Each row gives the match time, queue type, replay timestamp, champion, length, one takeaway, one fuller description, and a preview control. Rows are sorted with the newest match first and, within a match, the longest recording first. After manual storyboard review of the newest full recording, the current Samira target is ``name the payout before the dash'': before E/R, the player names wave, tower, dragon, recall, or nexus; if none is real, the dash waits. The row keeps evidence and nuance behind a compact disclosure so the advice remains tied to replay evidence without turning the first screen into a wall of coaching text.
