...
On May 18, 2026, the public page added a generated recordings manifest from the local League of Legends Highlights folder. The current manifest contains 14 recordings across matches \texttt{NA1-5563247854} and \texttt{NA1-5563301586}. The set totals 14:44 of video and includes short highlight clips plus two longer full-review recordings. Champion detection was bounded to visible replay evidence: sampled frames show Samira in the team list and center the Soul Fighter Samira model and nameplate. No other player champion was treated as source-confirmed for this recording set.

The recording review does not attempt to become a full coaching dashboard. The public surface now opens with the recording table because the recordings are the current queue plan; champion situations remain reference material below it. Each row gives the match time, queue type, replay timestamp, video, champion, length, one takeaway, and one fuller description. Rows are sorted with the newest match first and, within a match, the longest recording first. After manual storyboard review of the newest full recording, the current Samira target is ``reset before lethal HP'': when lane health drops to one enemy auto or spell, the wave is no longer the objective. The row also keeps the evidence basis, review limit, and nuance behind a compact disclosure so the advice is tied to replay evidence without turning the first screen into a wall of coaching text.
...
The website is served by a small Node application on Railway. It serves the hand-authored page, generated recording manifest, public recording files, poster frames, and paper artifacts from the public directory and exposes \texttt{/api/logs} for public note reads and writes. A local sync script scans the source Highlights folder, reads replay and League client logs for match and queue metadata, copies small \texttt{.webm} files into the public recording directory, compresses large recordings into deployable \texttt{.mp4} assets, extracts poster frames with \texttt{ffmpeg}, reuses cached feedback for unchanged files, and refreshes the manifest that the browser loads. The manifest records match time, queue type, replay timestamp, recording time, champion, duration, one takeaway, a fuller description, evidence basis, and review limit for each item, then sorts newest matches first and longest recordings first within each match. On Railway, persistent storage is provided through the configured volume path, and writes are gated by \texttt{LEAGUE\_WRITE\_TOKEN}. This keeps public notes browser-independent without adding a fake local-only note form or claiming Riot API integration that is not present in version 1.
...
The first public data source is the League practice arc supplied in the implementation thread and the public log exposed through \href{https://league.aolabs.io}{league.aolabs.io}. The current public page also exposes the May 18, 2026 recording manifest for \texttt{NA1-5563247854} and \texttt{NA1-5563301586}: twelve short highlight files, two full-review files, poster frames, local queue metadata, per-recording feedback, and one main queue focus. Riot API match history is not imported in this version.
