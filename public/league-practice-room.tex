...
The first public data source is the League practice arc supplied in the implementation thread and the public log exposed through \href{https://league.aolabs.io}{league.aolabs.io}. The current public page also exposes the May 18--21, 2026 recording manifest for 28 recordings across 16 local match identifiers, poster frames, local queue metadata, local client K/D/A and CS when available, per-recording feedback, one main queue focus, post-game queue status, and timestamp anchors that seek into the playable videos. The newest May 21 automatic review was corrected after frame inspection showed the unsupported reset critique should be narrowed to delayed base re-entry sync. Public Riot API match history is not imported in this version.
