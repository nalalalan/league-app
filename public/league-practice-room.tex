...
The recording review does not attempt to become a full coaching dashboard. The page gives one current improvement for the detected champion and one feedback note for every recording, but it no longer treats every clip as equal evidence. The manifest sorts recordings by source modified time, labels older clips as baseline evidence, labels later clips as implementation or current-form evidence, and weights the later clips more because they likely show the user trying to apply the previous feedback. For Samira, the current target is ``exit first, cash out second'': keep naming the exit before E, then stop the fight as soon as the map payoff is available. Each recording note also includes a short trust rationale so that the advice is tied to a visible replay pattern, Samira mechanic, map conversion rule, or concrete anxiety-reducing check rather than to generic motivation.
