...
Saved game-note reads & Preserve pasted game notes as readable documents while showing one all-entry takeaway, the June 30 onward game-time rank-vs-time chart with CS@10 on the right axis, parsed game date/time, rank, Alan/Samira-owned game facts, one cached model-backed description grounded in the note's actual analysis, one punctuation-preserving justified PDF action, and one delete action per note & Reject generic, inverted, repeated, report-like, overlong, cut-off, assistant-prefaced, or team-score-derived model text before it becomes card copy, then describe the visible saved-note surface, archived PDF routes, delete boundary, and source-bounded rank/CS@10 boundary \\
...
The page was constrained by user instructions gathered during planning and revision: one page only, clean and pretty, same AO Labs vibe, no poster hero, no dense dashboard, no redundant helper labels, no overexplaining, no bold card hierarchy when Alan rejects it, one quiet paper strip, and no visible recording or other-champion material when the current job is Samira note intake. Earlier paragraph reviews used green, red, and pink coach clauses, and a later review mode replaced them with a mistake table. Those systems are now source/archive layers, not the default home page. The current visible interface uses the shared AO Labs header, warm off-white palette, dark ink text, restrained borders, one visible paper action, one compact password-free Samira note input, one all-entry takeaway, a June 30 onward game-time rank-vs-time chart with CS@10 on the right axis, and dense saved-note reads showing parsed game date/time, rank, parsed game facts, one model-backed description grounded in the note's actual analysis, PDF action, and delete action. It avoids the rejected pattern of a large mostly empty intake card above cramped narrow note tiles. Card copy removes both role-prefix labels and colonless preface stems; the visible sentence should read like ``Name the climb-out after damage,'' not an improvement-wrapper around the same command. The model path has the same rule: if it copies a summary, misreads the note, or recommends the opposite behavior, the output is rejected before caching. The rank path is not model-owned: parsed Samira K/D/A, CS, CS@10, damage, gold, result, game type, and explicit rank phrases set the score, while note-only critique language cannot collapse the read to Iron and ordinary champion names cannot produce apex-rank outliers. The input width is independent from the chart and saved-note grid width. This interface description is treated as part of manuscript accuracy: when the page changes in a way that changes what Alan sees, how evidence is displayed, how queue state is communicated, or how coaching intent is encoded, the paper must be checked or updated in the same work cycle.
