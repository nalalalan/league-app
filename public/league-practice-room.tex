...
The older top review tile combined a selected champion's rank trend plot and latest synced state into one calm surface. That tile now acts as source history rather than the default interface. The current default card accepts long pasted Samira note blocks without a visible password field, stores them in the same persisted note corpus, and returns one all-entry takeaway plus saved notes with no PDF thumbnails: date, approximate rank, parsed game facts from the paragraph, one direct description that extracts the actual note analysis, one PDF action, and one delete action per note. The input and takeaway are not wrapped in one large blank panel; the note composer stays compact while the saved-note grid uses the page width. The takeaway is large enough to read as the current command, not as a small summary message stranded in empty space. Short saved notes do not stretch their buttons to the bottom of a tall empty card. On mobile, the paper strip compresses instead of forcing the intake downward into a stacked header block. The description is rendered through a direct-copy gate so stale stored or generated text cannot wrap the climb-out cue in an improvement preface when the tile should say ``Name the climb-out after damage.'' Older seed PDFs remain reachable by direct route, but they do not fill the home page. The aggregate Samira read and current tips remain available to \texttt{/api/samira} and the morning queue. The home page therefore answers one question first: where does the next Samira note go, and what rank-shaped pattern does the whole saved-note corpus imply?
