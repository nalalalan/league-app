...
The site organizes knowledge by situations rather than by abstract lessons. The current version makes those situations champion-specific: selecting Samira shows entries such as before pressing E, W timing, after R works, after a takedown, early ADC attacks, tower range, and damage panic. Selecting Caitlyn shifts the same practice-room idea toward next safe minion, staying unavailable, sustain after poke, and shooting the closest safe target. This format was chosen because it gives enough detail to review while avoiding the generic labels and motivational fragments that had made the first version feel unusable.

\begin{table}[H]
\centering
\caption{Example Samira situations imported into the public page.}
{\small
\setlength{\tabcolsep}{5pt}
...
\midrule
Before pressing E & Q first; check ratio and reachability & E as anxiety relief & Q creates; E commits \\
Low target in fog & Stop at edge; Q if visible & E into unknown bodies & Low plus reachable \\
W timing & Hold W for the real projectile & W as armor or tower block & Name one danger spell \\
After R works & Leave unless next kill is obvious & Stay from excitement & R is reward \\
After takedown & Check E and next target & Automatic reset dash & Kill one, check E \\
Damage panic & Step back and continue & Read damage as judgment & One hit is not the game \\
\bottomrule
\end{tabular}
}
\end{table}
