...
The older top review tile combined a selected champion's rank trend plot and latest synced state into one broad review surface. That tile now acts as source history rather than the default interface. The current default surface accepts long pasted Samira note blocks without a visible password field, stores them in the same persisted note corpus, and returns one all-entry takeaway, one June 30 onward rank-vs-time chart, and saved notes with no PDF thumbnails: date, approximate rank, parsed game facts from the paragraph, one direct description that extracts the actual note analysis, one PDF action, and one delete action per note. The input, takeaway, and chart are not wrapped in one large blank panel; the note composer stays compact while the chart uses a full-width row and the saved-note grid uses the page width. Short saved notes do not stretch their buttons to the bottom of a tall empty card. On mobile, the paper strip compresses instead of forcing the intake downward into a stacked header block. The description is rendered through a direct-copy gate so stale stored or generated text cannot wrap the climb-out cue in an improvement preface when the tile should say ``Name the climb-out after damage.'' Older seed PDFs remain reachable by direct route, but they do not fill the home page. The aggregate Samira read, rank trend, and current tips remain available to \texttt{/api/samira} from June 30 onward notes and current-window review data, and the morning queue uses that current read. The home page therefore answers one question first: where does the next Samira note go, and what rank-shaped pattern does the current saved-note corpus imply?
...
Saved game-note reads & Preserve pasted game notes as readable documents while showing one all-entry takeaway, the June 30 onward rank-vs-time chart, date, rank, parsed game facts, one direct description grounded in the note's actual analysis, one PDF action, and one delete action per note & Reject assistant preface stems in card copy and describe the visible saved-note surface, archived PDF routes, delete boundary, and source-bounded rank boundary \\
...
\texttt{/api/samira} read & Feeds rank estimate, rank-trend points, saved-note PDF metadata, and morning-email tips from June 30 onward Samira notes plus current-window review data & Describe the source-bounded rank/tip contract without showing recordings as a home-page dashboard \\
