...
The first public data source is the League practice arc supplied in the implementation thread and the public log exposed through \href{https://league.aolabs.io}{league.aolabs.io}. The current public page also exposes the May 18--21, 2026 recording manifest for 31 recordings across 18 local match identifiers, poster frames, local queue metadata, local client K/D/A and CS when available, per-recording feedback, one main queue focus, post-game queue status, and timestamp anchors that seek into the playable videos. The newest May 21 automatic reviews were corrected after frame inspection showed that unsupported generic reset critiques should be replaced with timestamped evidence about shutdown-lead conversion, base re-entry sync, spend-after-base-payout decisions, and queue/publish state. Public Riot API match history is not imported in this version.
