...
\href{https://league.aolabs.io}{league.aolabs.io} is a public AO Labs page built for that purpose. It is not a Riot Games product, a general League guide, or an official ranked-MMR system. The current home page is a Samira note intake, June 30 onward rank-vs-time chart, separate CS@10 chart, and dense saved-game note cards. Earlier Samira, Caitlyn, and Fizz review-room material remains source history. The design is source-bounded: it uses the user's supplied chat arc, public notes, local recordings, local client reads, and saved Samira note blocks as its corpus, and it avoids claims that would require clinical, population-scale, or competitive outcome evidence.

\section{Results}

\subsection{A long learning arc was reduced to a small review surface}

The seed corpus contained a long narrative account of League practice. The arc included lane fear, camera stability, sensory control, champion feel, bot practice, death tolerance, public yapping as pressure regulation, and champion-specific rules for Samira, Caitlyn, and Fizz. The current design result is not a long visible essay, a motivational hero, or a broad champion library. The page opens with a compact Samira work surface: a small width-capped note textarea, one all-entry takeaway, a rank-vs-time chart from parsed game date/time in June 30 onward saved note reads, and a separate CS@10 chart when pasted notes give that value. The rank chart stays a rank chart; CS@10 remains early-farm evidence instead of silently changing the rank score. The chart turns the former blank current-read lane into the primary visual object, while the takeaway stays a short command rather than a tiny caption inside a large empty card. Saved game-note cards follow in a grid wide enough to avoid skinny text towers. Each saved note shows parsed game date/time when present, approximate rank, parsed game type/result/KDA/CS/CS@10/damage details when present, one model-backed description that extracts the actual note analysis, PDF action, and delete action. The approximate rank is computed from explicit rank language first, including phrases such as ``gameplay-estimated rank for Alan's Samira performance: Gold II,'' then parsed Samira-owned facts and note language; champion names such as Master Yi cannot become rank points, and model text cannot override an explicit saved-note rank. When the model is unavailable or its output fails the visible-copy gate, the server uses a source-rule fallback. The description is not allowed to use assistant preface stems such as ``The improvement is'' or ``The leak is,'' and it is not allowed to carry rank-tier wording; rank belongs in the rank field.
...
The public page is not a neutral wrapper around the recordings. It is part of the intervention being studied. For that reason, the paper records the actual interface, not only the backend, manifest, or coaching text. The current live page begins with the shared AO Labs header, a quiet paper strip, one compact Samira note-intake composer, and saved-note PDF cards in a wide dense grid. The earlier champion-card surface used exact user-supplied photographs for Samira, Caitlyn, and Fizz, and those source assets remain in the project archive, but they are no longer part of the default home-page task. This matters because the page is meant to feel like a low-friction intake surface rather than a generic analytics product.

The older top review tile combined a selected champion's rank trend plot and latest synced state into one broad review surface. That tile now acts as source history rather than the default interface. The current default surface accepts long pasted Samira note blocks without a visible password field, stores them in the same persisted note corpus, and returns one all-entry takeaway, one June 30 onward game-time rank-vs-time chart, one separate CS@10 chart on the same note-time x-axis, and saved notes with no PDF thumbnails: parsed game date/time when present, approximate rank, Alan/Samira-owned game facts from the paragraph including CS@10 when present, one cached model-backed description that analyzes that note, one PDF action, and one delete action per note. The note PDF is generated from the saved source note whenever it is opened, includes the parsed game date/time when present, keeps apostrophes and other source punctuation visible instead of turning them into gaps, and lays paragraph lines against a fixed right edge so the saved note reads like a document rather than a ragged card export. The parser treats team scoreboard totals as context, not card metadata; when the pasted note says ``Team 1 won 42/23/55'' and ``Alan's Samira finished 16/6/5,'' the card shows 16/6/5. It also prefers final visible scoreboard Samira facts, \texttt{Alan/Samira:} lines, and post-fight score transitions over interim scoreboards. The rank chart is notes-only: full-game recording ranks remain archive evidence, but they cannot inject a chart point into the Samira intake. Free-text champion names cannot become rank labels. Explicit rank phrases in the note override cached model and derived scoring, including rank-for-performance sentences where the rank appears after the colon. CS@10 remains a parsed card fact and separate trend, not a hidden rank-score input, and it shares the rank chart's time domain so missing CS@10 on later notes appears as missing data rather than a shorter timeline. The input, takeaway, and charts are not wrapped in one large blank panel; the note composer stays compact while the charts use full-width rows and the saved-note grid uses the page width. Short saved notes do not stretch their buttons to the bottom of a tall empty card. On mobile, the paper strip compresses instead of forcing the intake downward into a stacked header block. The description is rendered through a direct-copy gate so stale stored or generated text cannot wrap the climb-out cue in an improvement preface when the tile should say ``Name the climb-out after damage.'' The same gate rejects generic model coaching, report-like rank reasons, inverted advice, repeated template paragraphs, rank-tier wording in descriptions, and cut-off text before the text is cached. Older seed PDFs remain reachable by direct route, but they do not fill the home page. The aggregate Samira read, rank trend, CS@10 trend, and current tips remain available to \texttt{/api/samira} from June 30 onward notes and parsed Samira game facts, and the morning queue uses that current read. The home page therefore answers one question first: where does the next Samira note go, and what rank-shaped pattern does the current saved-note corpus imply?
...
Saved game-note reads & Preserve pasted game notes as readable documents while showing one all-entry takeaway, separate June 30 onward game-time rank and CS@10 charts, parsed game date/time, rank, Alan/Samira-owned game facts, one cached model-backed description grounded in the note's actual analysis, one punctuation-preserving justified PDF action, and one delete action per note & Reject generic, inverted, repeated, report-like, rank-tier, overlong, cut-off, assistant-prefaced, or team-score-derived model text before it becomes card copy, then describe the visible saved-note surface, archived PDF routes, delete boundary, explicit-rank precedence, score-transition parsing, and separate CS@10 boundary \\
...
\texttt{/api/samira} read & Feeds rank estimate, game-time rank-trend points, saved-note PDF metadata, cached note-analysis engine state, and morning-email tips from June 30 onward Samira notes plus current-window review data & Describe the source-bounded rank/tip contract without showing recordings as a home-page dashboard \\
...
The public log starts with source entries for the seed arc import, the practice-room frame, the current Samira law, and later user-supplied Samira proof-game notes. The Samira intake stores longer pasted paragraphs in that same persisted note corpus and exposes them through a compact read endpoint for review, note PDFs, per-note rank reads, cached note analysis, and morning-email tips. The note PDF route reads the current saved note each time, so old saved notes receive the current punctuation and paragraph layout without rewriting the note record. This is intentionally public. The design choice is that thoughts, panic notes, and mindset rules are not private by default; they are part of the learning artifact. The only excluded material is unrelated sensitive information such as credentials, access tokens, or private third-party identifiers.

\section{Discussion}

The case suggests that the useful interface for some high-repetition learners is not less information in the abstract, but the right default object. A long learning conversation can contain many correct rules while still being hard to use before a game. The current home page therefore removes the champion picker, recording review room, archived situations, public note stream, older note archive, decorative PDF preview, and oversized enclosing intake panel from the default surface. The page now opens as one compact password-free Samira note textarea, save state, one all-entry takeaway, one June 30 onward game-time rank-vs-time chart, one separate CS@10 chart, and dense saved-note reads showing parsed game date/time, rank, Alan/Samira-owned game facts including CS@10 when present, one cached model-backed description grounded in the note's actual analysis, PDF action, and delete action. The note composer stays width-capped while the charts and saved-note grid use the wider page for many games, with card widths chosen to avoid narrow paragraph towers. The saved-note description is direct-action copy, not category-prefaced coaching prose or an evaluator summary; the server rejects generic model phrases, robotic rank reasons, rank-tier description leaks, team-score-derived player stats, and cut-off text, then falls back to source rules when the model cannot produce a useful read. Older PDFs, recordings, and distilled Samira tips remain source history until Alan reopens them. That is consistent with cognitive-load theory: reducing simultaneous elements can make practice more tractable \cite{sweller1988}. It is also compatible with mental practice research, because the stored notes can still become concrete game cues without making the capture surface feel like another review assignment \cite{driskell1994}.
...
The page was constrained by user instructions gathered during planning and revision: one page only, clean and pretty, same AO Labs vibe, no poster hero, no dense dashboard, no redundant helper labels, no overexplaining, no bold card hierarchy when Alan rejects it, one quiet paper strip, and no visible recording or other-champion material when the current job is Samira note intake. Earlier paragraph reviews used green, red, and pink coach clauses, and a later review mode replaced them with a mistake table. Those systems are now source/archive layers, not the default home page. The current visible interface uses the shared AO Labs header, warm off-white palette, dark ink text, restrained borders, one visible paper action, one compact password-free Samira note input, one all-entry takeaway, a June 30 onward game-time rank-vs-time chart, a separate CS@10 chart, and dense saved-note reads showing parsed game date/time, rank, parsed game facts, one model-backed description grounded in the note's actual analysis, PDF action, and delete action. It avoids the rejected pattern of a large mostly empty intake card above cramped narrow note tiles. Card copy removes both role-prefix labels and colonless preface stems; the visible sentence should read like ``Name the climb-out after damage,'' not an improvement-wrapper around the same command. The model path has the same rule: if it copies a summary, misreads the note, leaks rank-tier wording into the description, or recommends the opposite behavior, the output is rejected before caching. The rank path is not model-owned: explicit rank phrases in the saved note win first; parsed Samira K/D/A, CS, damage, gold, result, game type, \texttt{Alan/Samira:} lines, score transitions, and note language set the fallback score; CS@10 remains a separate charted early-farm fact; ordinary champion names cannot produce apex-rank outliers. The input width is independent from the charts and saved-note grid width. This interface description is treated as part of manuscript accuracy: when the page changes in a way that changes what Alan sees, how evidence is displayed, how queue state is communicated, or how coaching intent is encoded, the paper must be checked or updated in the same work cycle.
