...
The older top review tile combined a selected champion's rank trend plot and latest synced state into one calm surface. That tile now acts as source history rather than the default interface. The current default card accepts long pasted Samira note blocks, stores them in the same persisted note corpus, and returns today's newest unique saved note as a PDF card with an approximate rank read. Older note PDFs remain reachable by direct route, but they do not fill the home page. The aggregate Samira read and current tips remain available to \texttt{/api/samira} and the morning queue. The home page therefore answers one question first: where does the next Samira note go, and what rank-shaped pattern does that note imply?
...
Today note PDF card & Preserve the current pasted note as a readable document with its own approximate rank read & Describe the visible one-card surface, archived PDF routes, and the source-bounded rank boundary \\
