...
\href{https://league.aolabs.io}{league.aolabs.io} is a public AO Labs page built for that purpose. It is not a Riot Games product, a general League guide, or an official ranked-MMR system. It is a three-champion review room with a Samira note intake: Samira and Cait have source-confirmed recording pages, while Fizz has a clean empty recording page until Fizz footage exists. The design is source-bounded: it uses the user's supplied chat arc, public notes, local recordings, local client reads, and saved Samira note blocks as its corpus, and it avoids claims that would require clinical, population-scale, or competitive outcome evidence.
...
The top review tile combines the selected champion's rank trend plot and latest synced state into one calm surface. The plot uses exact per-game rank equivalents rather than broad ranges, and the state paragraph is latest-weighted: the newest synced full game carries the coaching state, the last three full games set the current pattern when enough footage exists, and the older archive median remains baseline context only. It is not live match telemetry; it is current through the latest generated recordings manifest. That top tile is intentionally not a broad dashboard. It exists to answer one question before the next game: what level is the latest synced play reading as, and what current pattern is most worth fixing next? The Samira intake below the selected-champion card accepts long pasted note blocks, stores them in the same public note corpus, and exposes a compact approximate rank read plus current Samira tips for the morning queue.
...
Recording card & Couples video with a short mistake table & Describe the Time/Mistake contract, five-row cap, category vocabulary, and clickable timestamp behavior \\
Archived coach clauses & Preserve earlier paragraph-review semantics for source history only & State that the current visible review no longer uses green/red/pink paragraphs unless the user reopens that mode \\
Samira intake and public log & Preserve pasted note blocks and older situation rules without competing with current review & State which material feeds the current tips and which material is demoted below the active recordings \\
\bottomrule
\end{tabular}
}
\end{table}
...
The site now organizes the visible page by Samira intake and recordings rather than by abstract lesson cards. Situation notes still exist because they explain the origin of the rules, but they are archived below the current recording review. Selecting Samira shows recording rows with the video and a compact mistake table; selecting Fizz shows the same page structure with no fake feedback because no Fizz footage has been source-confirmed. This format was narrowed because the user needed less cognitive load than a giant rule wall, a stat audit, or a paragraph coaching taxonomy.
...
The recording review does not attempt to become a full coaching dashboard. The public surface keeps champion cards first, then places one combined rank-and-state tile with a compact exact-rank trend plot and current selected-champion state. The recording rows below now use a two-column mistake table instead of a narrative paragraph: \emph{Time} gives the clickable game-clock anchor, and \emph{Mistake} gives one short allowed category plus the concrete wrong action and replacement action when visible. Rows are capped at five mistakes so the page answers the immediate question without forcing the user to parse rank explanations, paragraph color semantics, or internal audit language. The performance-rank estimate still exists in the manifest and trend plot, using Riot's current tier structure and queue distinctions \cite{riotRankedTiers2026,riotApexTiers2026}, but it is no longer repeated inside each visible review row unless the user asks for it. Co-op vs. AI rows remain lower-confidence ranked-habit evidence, while ranked/PvP full games remain the stronger transfer evidence. If the detailed model pass fails or returns no usable JSON, the sync builds a conservative table from verified clock frames and hides internal fallback wording from the visible review.
...
The page was constrained by user instructions gathered during planning and revision: one page only, clean and pretty, same AO Labs vibe, no poster hero, no dense dashboard, no redundant helper labels, no overexplaining, public notes by default, one quiet paper strip, a readable queue, and coaching that does not become another wall of work. Earlier paragraph reviews used green, red, and pink coach clauses, but the current visible review mode is intentionally simpler: a mistake table with \emph{Time} and \emph{Mistake}. The page therefore uses the shared AO Labs header, warm off-white palette, dark ink text, 8 px cards, restrained borders and shadows, compact fragments, one visible PDF action, and table rows that put timestamp and mistake category before any supporting archive detail. This interface description is treated as part of manuscript accuracy: when the page changes in a way that changes what Alan sees, how evidence is displayed, how queue state is communicated, or how coaching intent is encoded, the paper must be checked or updated in the same work cycle.
...
The website is served by a small Node application on Railway. It serves the hand-authored page, generated recording manifest, exact champion-card photos, poster frames, and paper artifacts from the public directory and exposes \texttt{/api/logs} for public note reads and writes plus \texttt{/api/samira} and \texttt{/api/samira/notes} for the Samira note/rank/tip read. Local recording media is served with byte-range responses so timestamp controls can seek directly into a video row. Large recording videos are kept out of the Railway upload payload; when a local video file is not present in the deployment image, the server redirects that recording request to a GitHub-hosted media copy for the same repository path. A local sync script scans the source Highlights folder, reads replay and League client logs for match and queue metadata, reads local League Client match history for champion id, K/D/A, CS, and game duration when available, copies or compresses recordings into deployable assets, extracts poster frames with \texttt{ffmpeg}, computes the research-calibrated rank trend, regenerates the selected-champion state, and refreshes the manifest that the browser loads. The current manifest review gate now requires a mistake table for full automatic reviews: one to five rows, each with a verified game-clock time and one allowed mistake category. The table gate rejects paragraph-only output, broad pressure-mode labels, rank explanations inside the visible review, and internal fallback/model wording. Samira rows audit green-light discipline first: if \texttt{W} readiness, above-half HP, or nearby ally presence is absent or not visibly proved, a forward champion commit is red light and the row must name the non-\texttt{E} input. The normal macro capture stays at 2 FPS so whole-game review remains low-lag and cheap, while the first 12 minutes can still be captured at a bounded higher frame rate for a separate early-lane micro pass. A separate timestamp and feedback audit rechecks the generated manifest before publication. A local live-recorder script watches for League game state, captures at low priority, queues one post-game review at a time while the next game can continue recording, and publishes queue status with per-stage and ready-time estimates. The manifest records match time, queue type, champion, game duration, K/D/A, CS, recording duration, the mistake table, optional early micro evidence, macro and micro capture FPS, exact rank estimate for trend plotting, evidence basis, and review limit for each item, then sorts newest matches first and longest recordings first within each match. On Railway, persistent storage is provided through the configured volume path, and writes are gated by \texttt{LEAGUE\_WRITE\_TOKEN}. This keeps public notes browser-independent without adding a fake local-only note form or claiming public Riot API integration that is not present in version 1.
...
The first public data source is the League practice arc supplied in the implementation thread and the public log exposed through \href{https://league.aolabs.io}{league.aolabs.io}. The current public page also exposes the generated recording manifest, poster frames, local queue metadata, local client champion id, K/D/A and CS when available, per-recording mistake tables, research-calibrated rank-equivalent macro estimates for trend plotting, the Samira note/rank/tip read, one main queue focus, compact post-game queue status, and timestamp anchors that seek into the playable videos. The manifest also exposes a rank-calibration block with the research sources, high-confidence requirements, and confidence blockers that separate ranked/PvP rows from bot-practice rows. The latest review-surface correction replaces paragraph reviews with timestamped mistake rows so unsupported generic reset critiques, reused drills, unverified entity labels, play-type template residue, rank explanations, and internal fallback text do not become the visible coaching surface. Public Riot API match history is not imported in this version.
