...
function manualFeedback(file) {
  if (file === "auto_NA1-5563660362_01.mp4") {
    return {
      champion: "Samira",
      confidence: "high",
      feedbackTitle: "Finish through the open base",
      feedback: "The win comes from repeating the open-base push; play second-entry, cash out structures, and skip chases that do not protect the end.",
      gameDetail: "Around 12:06, the game is already in the ending pattern: Garen is first into the broken base, enemies are trying to hold the doorway, and Samira is close enough to clean up without needing to start the fight. Around 13:06, Mordekaiser crosses mid and Samira turns with the group instead of peeling into a side chase, so the pressure keeps pointing back at the exposed base. By about 14:18, the frontline is back on structures and the enemy team is running out of stable defenders, so the win comes from repeating the same push, not finding a prettier fight. Master cue: when a base entrance is open, play Samira as the finisher behind the first body in, spend E only after the fight is already collapsing, and convert the won fight into structure before chasing.",
      whyTrust: "This is based on the visible ending sequence: the win does not come from a bigger Samira combo, it comes from staying close enough to finish the fight and cash out the open base.",
      ...
      diamondRule: "Open base means second-entry finisher first, structure conversion second, chase last.",
      drill: "Next game: when an inhibitor or nexus route is open, say first body, then E, then structure before pressing forward.",
      timeline: [
...
async function analyzeRecording({ file, duration, framePaths, frameTimes, sequenceLabel, reviewPhase: phase }) {
  ...
  const prompt = [
    ...
    "Write gameDetail like a short game-story recap, not a stat audit: one compact paragraph, one or two notable visible moments, at most three light timestamps, no K/D/A, no CS count, no numbered timeline, and one final Master-focus sentence.",
