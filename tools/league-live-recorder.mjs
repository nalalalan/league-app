...
const segmentSeconds = Number(process.env.LEAGUE_LIVE_SEGMENT_SECONDS || 10);
...
const fps = String(process.env.LEAGUE_LIVE_FPS || 4);
...
const liveCaptureCq = String(process.env.LEAGUE_LIVE_CAPTURE_CQ || 24);
const x264Crf = String(process.env.LEAGUE_LIVE_CRF || 20);
const x264CaptureCrf = String(process.env.LEAGUE_LIVE_CAPTURE_CRF || 36);
const captureBitrate = String(process.env.LEAGUE_LIVE_CAPTURE_BITRATE || "8000k");
const captureMaxrate = String(process.env.LEAGUE_LIVE_CAPTURE_MAXRATE || "10000k");
const captureBufsize = String(process.env.LEAGUE_LIVE_CAPTURE_BUFSIZE || "12000k");
const captureGop = String(Math.max(1, Math.round((Number(fps) || 4) * (Number(segmentSeconds) || 10))));
const captureScale = String(process.env.LEAGUE_LIVE_CAPTURE_SCALE || "1440:-2").trim();
